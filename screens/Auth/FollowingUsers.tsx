...
import { StyleSheet, Text, View, SafeAreaView, ScrollView, TouchableOpacity, LogBox } from "react-native";
...
export const FollowingUsersScreen = ({ route, navigation: { navigate } }) => {
    ...
    useEffect(() => {
        let isMounted = true;
          ...
          LogBox.ignoreLogs(["Can't preform a React state update"]);
            //users.map(item => user.following.includes(item.id) ? item.followed = true : item.followed = false);
        ...
        <SafeAreaView style={styles.container}>
            ...
            <View key={suser.id} style={styles.line}>
                <TouchableOpacity onPress={() =>
                    navigate('User',{ user: { id: suser.id, name:suser.name, username:suser.username, followed: true, img: suser.img }})
                }>
                    {suser.img != null ? 
